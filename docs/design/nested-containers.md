...
This design enables the `pi-coding-agent` to run containers *inside* its own container. The agent runs `podman
build`, `podman run`, `docker compose up`, and testcontainers-style integration tests. The design does not open a path around the mitmproxy interception layer or onto the host.
...
`--volume /var/run/docker.sock:/var/run/docker.sock` is the usual answer. This answer is
...
effectively host root. It does not weaken pi-container's isolation model. It deletes it.
This must never be offered, not even as an opt-in.

### Docker-in-Docker (`--privileged` daemon)

Classic DinD needs `--privileged` on the agent container. It uses all capabilities, all devices,
...
- Published ports land in the sidecar's netns, not the agent's, so `localhost:5432`
  from the agent does not reach them.
...
started from the built agent image. Both are now in
...
The agent runs its own rootless podman as `pi`. Containers it starts are **children**. They live
...
uid 0 to an unprivileged host user. The capability applies to what that namespace
...
originally promised. It is the main reason nesting must stay opt-in.
...
- **`containers-common` does not exist on Debian.** The package is
  ...
  This was moot in the end (see the last bullet), but `passt` and `uidmap` were only
  *Recommends* of `podman`. They always had to be listed by name.
- **`chmod u+s` on `newuidmap` does not work. `setcap cap_setuid+ep` does.** A
  ...
  `CapEff: 00000000800405fb`, euid 0). It still fails the kernel's id-map
  ...
  `cap_setuid=ep` (and the setuid bit removed) the identical map succeeds. The
  nested `podman info` reports `rootless=true`, `overlay`. This is also what
  `quay.io/podman/stable` does upstream. The original reasoning here had it
  backwards.
- **`nftables` is required for `compose`.** A plain nested `podman run` reaches the
  network through `pasta` and needs nothing extra. `docker compose` creates a
  user-defined network per project. netavark sets that bridge up itself. Without
  ...
  agent container runs a service. It sees the injected CA at `/etc/pi-container-ca.crt`.
  On the `--internal` network it still cannot reach the internet.
- **A `docker` shim ships after all** (open question 5): several tools shell out to
  `docker` by name. The shim is a one-line `exec podman "$@"` at
  `/usr/local/bin/docker`. A **compose provider** ships too: `podman-compose`.
  The system pip-installs it onto the base image's CPython (the apt package would pull in Debian's
  separate `python3`). The system pins it via `[engine] compose_providers` because podman's
...
- **`/dev/net/tun` + `NET_ADMIN` does not create egress.** The agent can build tunnels in
  its own netns. There is still exactly one way off the isolated network: the proxy. The
  proxy's `FORWARD` policy is `DROP`.
- **Flow attribution coarsens.** `flow_export` partitions captured flows by client IP.
  ...
  addressing on the isolated net. The sibling-sidecar design would need that too.

### 5. SELinux: the one real concession

The host runs SELinux in **Enforcing** mode. `/dev/net/tun` is inaccessible under the
default container label. The full matrix is measured:
...
container. It is the better posture. The agent stays confined. But it only works if
**every** nested container passes `--security-opt unmask=all`. That cannot be made
transparent. `unmask` is not a `containers.conf` key. A drop-in setting is silently
...
**Decision:** default to `label=disable`. Expose `security: engine_t` in config for
...
genuine reduction and belongs in the docs as one. This is why nesting is **opt-in and
off by default**.
...
system trust store. `build.sh` already installs the mitmproxy CA there. It works unmodified.

**Processes inside nested containers**: an arbitrary image (`alpine`, `postgres`) has no
idea about the mitmproxy CA. Every TLS call from inside it fails. The system fixes this with a
...
mitmproxy CA). Pointing `SSL_CERT_FILE` at it is strictly additive.
...
`/etc/ssl/certs/…` path in its own image still fails. The fix is per-image (`COPY` the
CA in a Dockerfile stage). This is unavoidable without rewriting arbitrary images.

### 7. Registry allowlist

The proxy's allowlist is `default_action: block`. Nesting is useless until you permit
registry hosts. Ship a **commented-out** block in the seeded
`allowlist.yaml`. Comment it out so enabling nesting is a deliberate two-step, matching how
...
Nested containers are processes in the agent's cgroup. `resources.agent.memory/cpus`
...
avoid a hard failure on the missing systemd/dbus. Inner limit flags are then
...
`resources.agent.memory` is `16g`. That mismatch is pre-existing. nested builds are the
workload most likely to expose it.

Since this design landed, **4 GB is the documented minimum** `podman machine` size. This is not for
nesting. It is for building the images at all. The CPython compile moved into the
shared base image. It moved into the toolchain image (`pi-coding-agent-builder/`), which also
compiles podman and netavark. `build.sh` checks available memory
in the VM before starting a build. It refuses with the fix rather than OOM-killing the
compiler minutes later. Enabling nesting warrants more again.
...
workloads (a dev server, a compose stack, a database admin UI). A good share of
those are only useful if a human can *look* at them. As designed, they could not be.
The agent container publishes no ports. A nested container's `-p 3000:3000` binds
...
The connection arrives with **the agent's own address as its source**. This is
also the address `pasta` hands the nested guest. Source and guest address collide.
The flow never resolves. pasta accepts, then stalls. On a *routed* network the
same setup works. The source is the bridge gateway instead. This is why this
did not surface in the original verification. The verification used a routed network for
everything except the no-bypass proof.

The address collision is not a bug being worked around. Assigning the guest the
host's own address is how pasta deliberately avoids NAT. Its documented
...
The measurements above are the evidence for it. `pasta -a <addr>` is the documented
knob for giving the guest a distinct address. It is what podman exposes as
`--network pasta:-a,<addr>`. The team tried it first. The inner container failed to
start with it.
...
Ports are declared rather than discovered. A container's published ports are
...
checked against `0.0.0.0`). It aborts the launch naming the conflict instead of
...
controls. An established TCP connection is bidirectional. `expose: localhost` is
the default. It keeps that to host-local processes. `lan` binds `0.0.0.0`. It is
...
container reaching a service in the *agent's* netns. Examples: a metrics exporter, a mock API, a
language server. Anything a project runs beside its stack rather than inside it. The
conventional address for that is `host.docker.internal`. Inside the agent it is
wrong.
...
`192.168.127.254` is gvproxy. It is the **macOS host** as seen from the podman machine VM.
Two independent things are wrong with it. The agent-side service is not on the host. The
agent's `--internal` network has no route to the host anyway. The second failure
masks the first. The connection times out instead of being refused. This presents as a
...
shows nothing. It shows no error anywhere in the UI.

The value is not computed for the nested netns. It is byte-identical to the agent's own
`/etc/hosts` entry. The agent carries `/run/.containerenv`. The nested podman detects
that it is itself running inside a container. It propagates the parent's entry.
...
`169.254.1.2` is podman's own. It comes from the rootless-netns setup inside the agent:

```
/usr/bin/pasta --config-net … --no-map-gw --map-guest-addr 169.254.1.2
```

`--no-map-gw` is why the bridge gateway refuses. `--map-guest-addr` is the address
that replaces it. Both are podman's choices for the rootless network namespace. This
...
accepts it and keeps its own entry too. The name resolves to two addresses:
...
busybox `wget` inside the container takes the first match and succeeds. This is
exactly the check an operator runs to confirm the fix, and it passes. Prometheus stayed
`down` anyway. It shows the same `context deadline exceeded` and a `lastScrapeDuration` of
exactly 1.0006s. That is the full `scrape_timeout`. Go's resolver sorts multi-address results by
RFC 6724 destination-address selection. It ranks global scope above link-local, so it
dials `192.168.127.254` first. That is a black hole that neither connects nor refuses. The
...
evidence. It rules out every other explanation for the timeout.

`--no-hosts` would suppress podman's entry but is mutually exclusive with `--add-host`.
The image's `/etc/hosts` has no such name to begin with. The name cannot be made
unambiguous. The fix is to stop resolving it. Put `169.254.1.2` literally wherever
...
documented `containers.conf` key for exactly this. It does not take effect here.
The system tried it under `[containers]` and under `[network]`. Both were ignored. The test controlled for
the obvious explanation. The same override file also set a marker `env` entry. That
*did* reach the container, so the file was read. Only the key was disregarded. That
is consistent with the `/run/.containerenv` propagation above outranking it. The team also rejected: rewriting `/etc/hosts` in the agent image (the agent's own entry is correct
*for the agent* — it really can reach the host through the proxy). It also rejected a per-project
`--add-host` injected by pi-container (there is no hook on the nested `podman run`. It
would be wrong for containers that legitimately mean the host).
...
meaningless outside a pi-container agent. A compose **named overlay** confines it. An
explicit `-f` list replaces auto-discovery. `docker-compose.pi-container.yml` is
inert for everyone who does not pass it. `docker-compose.override.yml` (loaded
automatically) would not be. That makes the fix reviewable and shared instead of
rediscovered per operator. Selecting it is what the `PI_CONTAINER` marker is for. "Forgot the overlay" stops being a failure mode rather than becoming a warning. The
...
well-contained under a userns. It is a credible host-escape surface without one. Shipping
...
It relies on `--network` ordering to land the isolated network on `eth1` because Docker
...
- `proxy_secondary_connect_argv()` — the abstract hook exists *only* because
  `docker run` attaches one network. The whole post-run `network connect` step in
...
runtime would attach. Collapsing it would be a large diff for no gain.
...
| Nested container bind-mounts `/workspace/...` | Works. nested containers are children in the agent's mount namespace. They cannot reach beyond what the agent already sees. |
...
Each phase leaves the tree working. The security-relevant surface all lands in phase 3.
...
1. ~~**Startup warning for un-allowlisted registries.**~~ Resolved. `run.py`'s
   ...
   is true. It warns both when no registry is present and when a registry is allowed
   without the CDN host its layer blobs redirect to. The blob-host map is verified
   against the live registries. It needs re-checking when one moves its CDN, as Docker
   Hub did (Cloudflare → CloudFront).
2. **Concurrent runs sharing one nested-storage volume.** Two agents in the same
   workspace both mount `pi-nested-<hash>`. Each has its own `XDG_RUNTIME_DIR`. podman's storage locks don't see each other. This is a plausible route to a corrupted store.
...
4. **Nested-container flow attribution.** Accepting the agent's IP for all nested flows
   ...
   containers addressable on the isolated network. This is a much larger change. It would reopen the sibling-sidecar tradeoff.
5. **Does `pi` need a `docker` alias?** Many tools shell out to `docker` specifically.
   *Answered: yes.* A one-line `exec podman "$@"` shim at `/usr/local/bin/docker` plus
   ...
   provided the shim but depends on its own podman 5.4.2. The shim is written in the
