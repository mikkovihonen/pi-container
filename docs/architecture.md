...
mitmproxy intercepts only **HTTP, HTTPS, and DNS**. The proxy applies the allowlist and token redaction to these protocols. The system denies every other protocol by default. The proxy's `iptables` `FORWARD` chain policy is `DROP`. The system permits the `llama-server` API explicitly. The egress policy is **per-project**. To let the agent use another protocol (uninspected, plain NAT), opt it in under `egress.allow` in the workspace's `.pi-container/config.yaml`. The system seeds this file with deny-all on first run:
...
set of a packaged binary is an accident of the packager's environment. You set the tags
explicitly. This makes the feature set a deliberate choice. The choice for a rootless podman running
...
- **`containers_image_openpgp`** — Go-native OpenPGP instead of cgo `gpgme` plus the
  separate `podman-sequoia` library. This drops two runtime dependencies that the
  signature policy for this image does not use.
...
is not a downgrade. The mode uses the same Node with the version pinned here instead of in a
tag. `NODE_SOURCE=build` compiles from source. The build gives a trixie-native build rather than
...
`/python/` alone**. The proxy gets the same CPython and the same `uv` the agent runs. The proxy gets no Node,
...
Two further consequences are worth knowing:
...
- **Project-specific image rebuilds never compile anything.** Before the split, a
  ...
  compile. Now the rebuild is a file copy.
- **A rebuilt builder invalidates project images**, the same way a rebuilt proxy
  ...
  both values (see `_newest_shared_image_time()`).
...
second `uv` pin next to the agent's. A different file bumped them by hand, and the pins drifted apart. The proxy could run a CPython patch release, an OpenSSL build, or a `uv` binary that the
...
- The proxy's base is plain `debian:trixie-slim` plus the runtime shared libraries that the
  staged CPython's extension modules link against. A missing library causes a **build**
  failure. `python -c 'import bz2, ctypes, curses, lzma, readline, sqlite3, ssl,
  zlib'` runs right after the `COPY`. The test catches the failure early. This approach avoids letting `import ssl` fail at
  interception time.
...
Both images now carry the same CPython install. Every megabyte of CPython costs twice. `build-python.sh` trims the staged tree of what neither image can
reach. The script takes the size from 474 MB to 175 MB:
...
This is the trim that the official `python:` images make as well, minus their `*.pyc`
...
What the image gives up is **embedding CPython in a C/C++ program**. `python-config --ldflags
...
the interpreter core into the `python3.14` binary. Tools that require `libpython` could not work in this image before the trim and
...
`--enable-shared` rebuild. The rebuild costs every workload a slightly slower interpreter to
...
`uv sync` building a wheel from source in a workspace. The build compiles a small C extension against the staged headers. The build imports the extension with the staged interpreter. The build creates a `python -m venv` to confirm
`ensurepip` and the stdlib came through. A failure there fails the build.

Net effect on the images: proxy 417 MB → 509 MB, agent 1.6 GB → 1.3 GB, builder 790 MB → 486 MB. The proxy gained a full interpreter and lost the slim base. The numbers are the final image sizes.

**Build order is therefore a dependency, not a preference:** builder → proxy → agent.
`build.py` builds them in that order. The proxy `COPY`s from the builder. The
...
`build.sh` builds this image. Nothing else builds the image.
...
is the toolchain version history. Any pin can be overridden for a one-off build:
...
The Rust pin is the one exception to per-stage declaration. The pin is declared *before* the
...
default. This is how a pre-`FROM` ARG is inherited. The approach gives one pin rather than two that can
drift apart.
