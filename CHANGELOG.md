...
### Fixed
- **The proxy no longer installs packages from PyPI every time it starts.** Startup had grown slow enough to trip `run.py`'s 30s proxy health probe. The entrypoint launched mitmproxy with `uv run mitmweb`, and `uv run` reconciles `.venv` against the project's *default* dependency groups (`[tool.uv] default-groups = ["src", "proxy", "dev"]`) before it execs anything — while the image built the venv with `--only-group proxy --no-dev`. So every container start found a mismatch and downloaded the missing `src`/`dev` packages: `ruff` (10.4 MiB), `virtualenv` via `pre-commit`, `hf-xet` via `huggingface_hub`, `pygments` via `pytest`. Measured at ~15s on a warm connection, unbounded on a cold or slow one, and the timeout landed *after* the images were built, so a launch failed at the last step. The entrypoint now execs `/home/mitmproxy/.venv/bin/mitmweb` directly: no `uv` at run time, no network before mitmproxy exists to inspect it, and the running environment is exactly the frozen one the build produced. Health probe now answers in ~0.3s.
- **The venv the proxy actually runs is now complete.** The runtime `uv run` sync was load-bearing without anyone intending it to be: `--only-group proxy` installs that group *alone*, excluding the project's own `dependencies`, so the built venv had no `pyyaml` — which `allowlist.py` and `token_replacer.py` both import at module load. Nothing caught it because the sync at startup happened to install it, and the image's smoke test (`timeout 3s mitmweb`) loads no addon scripts. Cutting the sync would have shipped a proxy whose allowlist died on `ModuleNotFoundError` — fail-open on the component whose whole job is to fail closed. The build now runs `uv sync --frozen --no-default-groups --group proxy` (project dependencies + mitmproxy, no `src`/`dev` tooling), and a second smoke test starts `mitmdump` as the `mitmproxy` user with all three addons and the baked configs, placed after the `ENV` lines so the addons resolve the same config paths they will at run time. A missing addon dependency, an unreadable script, or a config that does not parse is now a build failure instead of a silent runtime one.
