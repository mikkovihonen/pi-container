...
### Added
- **Comprehensive security hardening configuration (`security:`).** Added a dedicated `security:` section to `config.yaml` and `schema_common.py` providing defense-in-depth safeguards:
    - `security.read_only_git_hooks` (default: `true`): Automatically mounts `/workspace/.git/hooks` as read-only (`:ro`) inside the container, preventing prompt-injected or untrusted agent code from planting malicious Git hooks that would execute with native developer privileges on the host machine.
    - `security.blocked_mount_paths`: Validates extra bind mounts configured in `agent.mounts` against a list of dangerous host directories (`~/.ssh`, `~/.gnupg`, `~/.aws`, `~/.azure`, `~/.config/gcloud`, `~/.kube`, `~/.docker`, `/var/run/docker.sock`, `/var/run/podman/podman.sock`, `/etc/shadow`, `/etc/passwd`, `/etc/sudoers`, `/root`), failing fast with an actionable security error if any are requested.
    - `security.blocked_ip_ranges`: Enforces proxy-level SSRF and DNS rebinding protection blocking connections to loopback interfaces, private subnets (RFC 1918), link-local addresses, and cloud instance metadata services (`169.254.169.254`) unless explicitly allowlisted, while keeping local model providers (`llama`) securely accessible.
    - `security.git_config_allowlist`: Enforces strict allowlist filtering on host Git configuration forwarded to the container (`user.name`, `user.email`, `init.defaultBranch`, `pull.rebase`, `core.autocrlf`, `core.filemode`, `core.eol`, `commit.gpgSign`, `user.signingKey`, `gpg.format`, `gpg.ssh.program`, `tag.gpgSign`), and unconditionally strips execution hooks (`core.sshCommand`, `core.fsmonitor`, `core.editor`, `filter.*`) and `http.extraHeader`.
    - **Privilege escalation hardening**: Automatically applies `--security-opt no-new-privileges` to agent and proxy containers, and mounts named shadow volumes and `tmpfs` RAM disks with `nodev,nosuid` to prevent setuid privilege escalation.
...
- **Orphaned proxy container & network crash recovery sweeper.** Added `ContainerNetworkManager.cleanup_orphaned_proxies()` and `containers.sweep_orphaned_proxies()`, called during startup crash recovery in `src/run.py` to automatically discover and reap orphaned `pi-proxy-<hash>` containers and `pi-net-<hash>` bridge networks left behind after ungraceful crashes or hard kills.
- **Parallel dependency startup for llama-server and proxy.** Added `containers.start_dependencies_parallel()` and `Server.port_ready_event`, allowing multiple `llama-server` instances and the `pi-proxy` container to start and initialize concurrently during `pi` startup. The agent container launches once all dependencies report healthy, reducing startup latency from $\sum \text{dependencies}$ to $\max(\text{dependencies})$.

### Changed
- **Unified refcount lifecycle management and concurrency hardening.** Extracted resource reference counting and lifecycle state machines into reusable `start_refcounted_resource()` and `stop_refcounted_resource()` helpers in `src/util.py`, eliminating duplicate logic across `src/server.py` and `src/network.py`.
- **Atomic client manifest writes and single source of truth.** Updated `write_clients()` to write atomically via tempfile replacement, and fixed `get_ref_count()` to use `clients.json` as the single source of truth (preventing false fallbacks to stale integer `.refcount` files when all clients are dead). Added `file_lock()` context manager with configurable timeout and contention logging to prevent deadlocks.
- **Migrated documentation site from MkDocs to Zensical.** Replaced `mkdocs-material` with `zensical>=0.0.57` in the `docs` dependency group, converted configuration from `mkdocs.yml` to native `zensical.toml`, updated GitHub Pages CI workflow to use `zensical build --clean`, and fixed anchor link resolution in documentation files.
...
- **Project-configurable proxy memory management and flow trimming.** Added `proxy.max_view_flows` (default: `2000`) and `proxy.stream_large_bodies` (default: `"10m"`) to `config.yaml` and `schema_common.py`. `FlowExporter` now trims allowed in-memory flows when exceeding the limit while **always preserving blocked/denied flows** (403, 444, connection errors) in `mitmweb`'s view for UI inspection.
...
- **Chat template relative path resolution.** Updated `src/server.py` to resolve relative `--chat-template-file` paths against `repo_root`, ensuring chat templates load consistently regardless of the host working directory.
- **Container secret exposure in process table.** Updated `_env_flags()` in `src/network.py` to pass `--env KEY` without `=VALUE` and injected secrets via the process environment dictionary, hiding secret tokens from CLI `argv` and process tables.
- **Orphaned agent container name cleanup across container runtimes.** Stripped leading slashes (`name.lstrip("/")`) in `src/run.py` to ensure container names are matched consistently across Docker and Podman JSON output.
- **Repository hygiene and runtime cache leaks.** Removed committed runtime files (`.claude/settings.local.json`, `.pi-container/agent/trust.json`, and `.pi-container/agent/models-store.json`) from the git index and added `.pi-container/agent/trust.json` and `.pi-container/agent/models-store.json` to `.gitignore`.
