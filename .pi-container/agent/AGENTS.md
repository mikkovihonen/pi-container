...
Whenever encountering an unmet system package dependency, the correct course of action is to append the dependency into `/workspace/.pi-container/dependencies/apt/packages.txt`. The system is using `apt` package management. After appending, the agent must stop and inform the user that a new dependency has been identified.
